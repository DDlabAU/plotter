...
function setup(){
  createCanvas(xDim, yDim, SVG); // the last parameter allows us to save the canvas as an SVG
  ...
  fillColor = color(fillColor = color(floor(random(0,255))));
...
function draw(){
  ...
  if(currentMillis > timer){
    ...
    if(sentences >= maxSentences){  //reach the max for each asterisk
      ...
      sentences = 0;
      fillColor = color(floor(random(0,255)));
    }
    pop();  //restore previous state
  }

  if (itr == 1){
    save("asterisk_grey.svg"); // Write .svg or .png to save the canvas (use svg for plotter and png for laser)
    print("Image saved");
